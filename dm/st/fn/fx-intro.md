...
The concept of function was formalized at the end of the XIX century in terms of set theory, which greatly enlarged the function's domains of application. A function was defined as a special kind of relation on sets.

> A function is a relation on sets that associates each element $$x$$ of a set $$X$$, called the **domain** of the function, to a single element $$y$$ of a set $$Y$$, called the **codomain** of the function.

Sets $$X$$ and $$Y$$ may be the same set. Consider the function $$f(x)=x^2$$ from $$\mathbb {R}$$ to $$\mathbb{R}$$. Its graph is the set of points $$R=\{(x,x^2) : x\in R\} \subseteq \mathbb {R}\times \mathbb {R}$$
